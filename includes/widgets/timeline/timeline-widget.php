...
class MgAddon_Timeline_Widget extends \Elementor\Widget_Base
{
	...
	{
		return ['mg-timeline', 'mg-timeline-active'];
	}
...
?>
		...
		{
			?>
				<# var mgtl_title_show=settings.mgtl_title_show || '' , mgtl_desc_show=settings.mgtl_desc_show || '' , mgtl_date_show=settings.mgtl_date_show || '' , mgtl_title_tag=settings.mgtl_title_tag || 'h4' , mgtl_date_position=settings.mgtl_date_position || '' , mgtl_list=settings.mgtl_list || [], mgtl_mod=settings.mgtl_mod || '' ; if (mgtl_mod==='horizontal' ) { var mgtl_hs_position=settings.mgtl_hs_position || '' , mgtl_hs_startIndex=settings.mgtl_hs_startIndex || '0' , mgtl_slide_number=settings.mgtl_slide_number || '1' , mgtl_visible_items=settings.mgtl_visible_items || '4' ; #>
					<div class="mg-timeline timeline" data-mode="horizontal" <# if (mgtl_hs_position==='bottom' ) { #>data-horizontal-start-position="bottom"<# } #>
							<# if (mgtl_hs_startIndex !=='0' ) { #> data-start-index="{{ mgtl_hs_startIndex }}"<# } #>
									<# if (mgtl_slide_number !=='1' ) { #> data-move-items="{{ mgtl_slide_number }}"<# } #>
											<# if (mgtl_visible_items !=='4' ) { #> data-visible-items="{{ mgtl_visible_items }}"<# } #>
													>
													<# } else { var mgtl_vst_position=settings.mgtl_vst_position || '' , mgtl_v_trigger=settings.mgtl_v_trigger || { size: '15' , unit: 'px' }; #>
														<div class="mg-timeline timeline" data-mode="vertical" <# if (mgtl_vst_position==='right' ) { #>data-vertical-start-position="right"<# } #>
																<# if (mgtl_v_trigger.size !=='15' || mgtl_v_trigger.unit !=='px' ) { #>
																	data-vertical-trigger="{{ mgtl_v_trigger.size }}{{ mgtl_v_trigger.unit }}"
																	<# } #>
																		>
																		<# } #>
																			...
																			<div class="timeline__wrap">
																				<div class="timeline__items">
																					<# _.each(mgtl_list, function(value, index) { var key1=view.getRepeaterSettingKey('mgtl_title', 'mgtl_list' , index), key2=view.getRepeaterSettingKey('mgtl_desc', 'mgtl_list' , index), key3=view.getRepeaterSettingKey('mgtl_date', 'mgtl_list' , index); view.addInlineEditingAttributes(key1); view.addInlineEditingAttributes(key2); view.addInlineEditingAttributes(key3); view.addRenderAttribute(key1, 'class' , 'mgt-head' ); #>
																						<div class="timeline__item">
																							<div class="timeline__content">
																								<# if (value.mgtl_date && mgtl_date_show==='yes' && mgtl_date_position==='top' ) { #>
																									<div class="mgt-time"><span {{{ view.getRenderAttributeString(key3) }}}>{{{ value.mgtl_date }}}</span></div>
																									<# } #>

																										<# if (value.mgtl_title && mgtl_title_show==='yes' ) { #>
																											<{{{ mgtl_title_tag }}} {{{ view.getRenderAttributeString(key1) }}}>{{{ value.mgtl_title }}}</{{{ mgtl_title_tag }}}>
																											<# } #>

																												<# if (value.mgtl_date && mgtl_date_show==='yes' && mgtl_date_position==='middle' ) { #>
																													<div class="mgt-time"><span {{{ view.getRenderAttributeString(key3) }}}>{{{ value.mgtl_date }}}</span></div>
																													<# } #>

																														<# if (value.mgtl_desc && mgtl_desc_show==='yes' ) { #>
																															<div class="mgt-content">
																																<p {{{ view.getRenderAttributeString(key2) }}}>{{{ value.mgtl_desc }}}</p>
																															</div>
																															<# } #>

																																<# if (value.mgtl_date && mgtl_date_show==='yes' && mgtl_date_position==='bottom' ) { #>
																																	<div class="mgt-time"><span {{{ view.getRenderAttributeString(key3) }}}>{{{ value.mgtl_date }}}</span></div>
																																	<# } #>
																							</div>
																						</div>
