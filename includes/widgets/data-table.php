		...
		$repeater->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} {{CURRENT_ITEM}} .mg-table--head-column-cell-icon svg' => 'fill: {{VALUE}}',
				],
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} {{CURRENT_ITEM}} .mg-table--body-row-cell-icon svg' => 'fill: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mg-table--head-column-cell-icon svg' => 'fill: {{VALUE}}',
				],
			]
		);
...
/*
	...
	protected function content_template() {
		...
		_.each(settings.rows_data, function(row) {
			var row_id = _.uniqueId();
			...
			if (row.row_column_type === 'column') {
				var table_row_keys = _.keys(table_row);
				var cell_key = _.last(table_row_keys);
	
				table_cell.push({
					'repeater_id': row._id,
					'row_id': table_row[cell_key]['id'] !== undefined ? table_row[cell_key]['id'] : '',
					'title': row.cell_name,
					'row_span': row.row_span,
					'row_column_span': row.row_column_span,
					'row_icon': !_.isEmpty(row.row_icon) ? row.row_icon : '',
					'row_icons': !_.isEmpty(row.row_icons.value) ? row.row_icons : '',
					'row_image': _.has(row, 'row_image') ? row.row_image : '',
					'row_thumbnail_size': !_.isEmpty(row.row_thumbnail_size) ? row.row_thumbnail_size : '',
					'cell_link': !_.isEmpty(row.cell_link.url) ? row.cell_link : ''
				});
			}
		});
		#>
		...
		<table class="mg-data-table mg-table item-visiable">
			<thead class="mg-table--head">
				<tr class="mg-table--head-column">
					<#
					_.each(settings.columns_data, function(column_cell, index) {
						var column_repeater_key = view.getRepeaterSettingKey('column_span', 'columns_data', index);
						view.addRenderAttribute(column_repeater_key, 'class', 'mg-table--head-column-cell');
						view.addRenderAttribute(column_repeater_key, 'class', 'elementor-repeater-item-' + column_cell._id);
	
						if (column_cell.column_span) {
							view.addRenderAttribute(column_repeater_key, 'colspan', column_cell.column_span);
						}
					#>
						<th {{{ view.getRenderAttributeString(column_repeater_key) }}}>
							<div class="mg-table--head-column-cell-wrap">
								<div class="mg-table--head-column-cell-text">{{{ column_cell.column_name }}}</div>
								<#
								if (column_cell.column_media === 'icon' && !_.isEmpty(column_cell.column_icons)) { #>
									<div class="mg-table--head-column-cell-icon">
										<i class="{{{ column_cell.column_icons.value }}}"></i>
									</div>
								<# } #>
								<#
								var image_url = '';
	
								if (!_.isEmpty(column_cell.column_image.url) || !_.isEmpty(column_cell.column_image.id)) {
									var image = {
										id: column_cell.column_image.id,
										url: column_cell.column_image.url,
										size: column_cell.column_thumbnail_size,
										dimension: column_cell.column_thumbnail_custom_dimension,
			...
			<tbody class="mg-table--body">
            <# for (var i = 0; i < settings.table_row.length; i++) { #>
                <tr class="mg-table--body-row">
                    <#
                    for (var j = 0; j < settings.table_cell.length; j++) {
                        if (settings.table_row[i]['id'] === settings.table_cell[j]['row_id']) {
                            var row_span_repeater_key = view.getRepeaterSettingKey('row_span', 'rows_data', settings.table_cell[j]['row_id'] + i + j);
                            view.addRenderAttribute(row_span_repeater_key, 'class', 'mg-table--body-row-cell');
                            view.addRenderAttribute(row_span_repeater_key, 'class', 'elementor-repeater-item-' + settings.table_cell[j]['repeater_id']);
                            if (!_.isEmpty(settings.table_cell[j]['row_column_span'])) {
                                view.addRenderAttribute(row_span_repeater_key, 'colspan', settings.table_cell[j]['row_column_span']);
                            }
                            if (!_.isEmpty(settings.table_cell[j]['row_span'])) {
                                view.addRenderAttribute(row_span_repeater_key, 'rowspan', settings.table_cell[j]['row_span']);
                            }
    
                            // link
                            if (!_.isEmpty(settings.table_cell[j]['cell_link']['url'])) {
                                var row_link_key = view.getRepeaterSettingKey('cell_link', 'rows_data', settings.table_cell[j]['row_id'] + i + j);
                                view.addLinkAttributes(row_link_key, settings.table_cell[j]['cell_link']);
                            #>
                                <td {{{ view.getRenderAttributeString(row_span_repeater_key) }}}>
                                    <div class="mg-table--body-row-cell-wrap">
                                        <div class="mg-table--body-row-cell-text">
                                            ...
                                            <#
                                            if (!_.isEmpty(settings.table_cell[j]['cell_link']['url'])) { #>
                                                <a {{{ view.printRenderAttributeString(row_link_key) }}}>
                                                    {{{ table_cell[j]['title'] }}}
                                                </a>
                                            <# } else {                                                
                                                {{{ table_cell[j]['title'] }}}
                                        ...
                                        <#
                                        if (!_.isEmpty(settings.table_cell[j]['row_icons'])) { #>
                                            <div class="mg-table--body-row-cell-icon">
                                                <i class="{{{ settings.table_cell[j]['row_icons']['value'] }}}"></i>
                                            </div>
                                        <# } #>
                                        <#
                                        if (!_.isEmpty(settings.table_cell[j]['row_image']['url']) || !_.isEmpty(settings.table_cell[j]['row_image']['id'])) {
                                            var table_cell_image_url = elementor.imagesManager.getImageUrl(settings.table_cell[j]['row_image']['id']);
                                            if (!table_cell_image_url) {
                                                return;
                                            }
                                        #>
                                            <div class="mg-table--body-row-cell-icon">
                                                <img src="{{{ table_cell_image_url }}}">
                                            </div>
                                        <# } #>
                                    </div>
                                </td>
