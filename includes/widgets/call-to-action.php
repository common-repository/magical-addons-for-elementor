...
?>
    ...
    {
    ?>
        <# var mg_cta_styles=settings.mg_cta_styles; //Call to action title var mg_cta_title=settings.mg_cta_title; var mg_cta_title_tag=settings.mg_cta_title_tag; var mg_cta_highlight=settings.mg_cta_highlight; view.addRenderAttribute('mg_cta_title', 'class' , 'mg-cta-title' ); //Call to action description var mg_cta_desc=settings.mg_cta_desc; view.addInlineEditingAttributes('mg_cta_desc', 'none' ); // Btn item var mgctaborder_show=settings.mgctaborder_show; var mgcta_border_position=settings.mgcta_border_position; var mg_flip_btn_use=settings.mg_flip_btn_use; var mg_flip_btn2_use=settings.mg_flip_btn2_use; var mgcta_btn_align=settings.mgcta_btn_align; var mgCtaImage={ id: settings.mgcla_img.id, url: settings.mgcla_img.url, size: settings.thumbnail_size, dimension: settings.thumbnail_custom_dimension, model: view.getEditModel() }; var mgcta_image_url=elementor.imagesManager.getImageUrl( mgCtaImage ); #>

            <# if (mg_cta_styles==='style1' ) { #>

                <div class="mg-cta style-one mg-shadow mgcta-{{ mgcta_btn_align }} mgcta-border-{{ mgctaborder_show }}-{{ mgcta_border_position }}">
                    <div class="mg-cta-content">
                        <# if ( settings.mg_cta_title ) { #>
                            <{{ mg_cta_title_tag }} {{{ view.getRenderAttributeString('mg_cta_title') }}}>
                                {{{ mg_cta_title }}}
                                <span>{{{ mg_cta_highlight }}}</span>
                            </{{ mg_cta_title_tag }}>
                            <# } if ( mg_cta_desc ) { #>
                                <p {{{ view.getRenderAttributeString('mg_cta_desc') }}}>{{{ mg_cta_desc }}}</p>
                                <# } #>
                    </div>
                    <div class="mg-cta-btn">

                        <# if ( settings.mg_flip_btn_use ) { mgf_button(settings, 1); } #>
                            <# if ( settings.mg_flip_btn2_use ) { mgf_button(settings, 2); } #>
                ...
                <# } else { #>
                    ...
                    <# } #>
                        ...
                        <# var settings=settings; function mgf_button(settings, btn=1) { var btn_attr='' ; if (btn===1) { var mg_flipbtn_icon_position=settings.mg_flipbtn_icon_position; var mg_flip_usebtn_icon=settings.mg_flip_usebtn_icon; var mg_flip_usebtn2_icon='' ; var mg_flip_btn_title=settings.mg_flip_btn_title; var mg_flip_btn_selected_icon=settings.mg_flip_btn_selected_icon; btn_attr='class="mg-flip-btn mgcla-btn"' ; var btn1Link=settings.mg_flip_btn_link; if ( btn1Link ) { btn_attr +=' href="' + btn1Link.url + '"' ; if ( btn1Link.is_external ) { btn_attr +=' target="_blank"' ; } if ( btn1Link.nofollow ) { btn_attr +=' rel="nofollow"' ; } } } else { var mg_flipbtn_icon_position=settings.mg_flipbtn2_icon_position; var mg_flip_usebtn2_icon=settings.mg_flip_usebtn2_icon; var mg_flip_usebtn_icon='' ; var mg_flip_btn_title=settings.mg_flip_btn2_title; var mg_flipbtn2_link_type=settings.mg_flipbtn2_link_type; if(mg_flipbtn2_link_type=='video' ){ var mg_flip_btn_link=settings.mg_flip_btn2_video_link; } else { var mg_flip_btn_link=settings.mg_flip_btn2_link; } var mg_flip_btn_selected_icon=settings.mg_flip_btn2_selected_icon; btn_attr='class="mgcla-btn2"' ; var btn2Link=settings.mg_flip_btn2_link; if ( btn2Link ) { btn_attr +=' href="' + btn2Link.url + '"' ; if ( btn2Link.is_external ) { btn_attr +=' target="_blank"' ; } if ( btn2Link.nofollow ) { btn_attr +=' rel="nofollow"' ; } } if (mg_flipbtn2_link_type=='video' ) { view.addRenderAttribute('mg_flip_btn_title', 'class' , 'mgcla-btn2-veno' ); view.addRenderAttribute('mg_flip_btn_title', 'data-autoplay' , 'true' ); view.addRenderAttribute('mg_flip_btn_title', 'data-vbtype' , 'video' ); } } if ((mg_flip_usebtn_icon==='yes' && btn===1) || (mg_flip_usebtn2_icon==='yes' && btn===2)) { #>
                            <a {{{ btn_attr }}}>
                                <# if (mg_flipbtn_icon_position==='left' ) { #>
                                    <span class="left"><i class="{{{ mg_flip_btn_selected_icon.value }}}"></i></span>
                                    <# } #>
                                        <span>{{{ mg_flip_btn_title }}}</span>
                                        <# if (mg_flipbtn_icon_position==='right' ) { #>
                                            <span class="right"><i class="{{{ mg_flip_btn_selected_icon.value }}}"></i></span>
                                            <# } #>
                            </a>
                            <# } else { #>
                                <a {{{ btn_attr }}}>{{{ mg_flip_btn_title }}}</a>
                                <# } #>
