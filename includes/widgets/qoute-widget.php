	...
	protected function register_style_controls()
	{
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mgq-quote-icon svg' => 'width: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'mgq_icon_type' => 'icon'
				]
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mgq-quote-icon svg' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} figure.mg-quote-img img,
				 {{WRAPPER}} .mgq-quote-icon i, {{WRAPPER}} .mgq-quote-icon svg'
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mgq-quote-icon svg' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} figure.mg-quote-img img, {{WRAPPER}} .mgq-quote-icon i, {{WRAPPER}} .mgq-quote-icon svg'
			]
		);
		$this->add_control(
			'mgq_icon_color',
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mgq-quote-icon svg' => 'fill: {{VALUE}}',
				],
				'condition' => [
					'mgq_icon_type' => 'icon'
				]
		...
		$this->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} figure.mg-quote-img img, {{WRAPPER}} .mgq-quote-icon i,
					 {{WRAPPER}} .mgq-quote-icon svg' => 'background-color: {{VALUE}};',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mcq-icon-top svg' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mcq-icon-top svg' => 'fill: {{VALUE}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mcq-icon-top svg' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mcq-icon-bottom svg' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __('Bottom Icon Color', 'magical-addons-for-elementor'),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .mcq-icon-bottom i' => 'color: {{VALUE}};',
					'{{WRAPPER}} .mcq-icon-bottom svg' => 'fill: {{VALUE}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				'label' => __('Bottom Icon Size', 'magical-addons-for-elementor'),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => ['px'],
				'selectors' => [
					'{{WRAPPER}} .mcq-icon-bottom i' => 'font-size: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .mcq-icon-bottom svg' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
...
<?php
	...
	protected function content_template() {
		?>
		<#
		var mgq_style = settings.mgq_style;

        var mgBlockImage = {
			id: settings.mgq_type_image.id,
			url: settings.mgq_type_image.url,
			size: settings.mgq_thumbnail_size,
			dimension: settings.mgq_thumbnail_custom_dimension,
			model: view.getEditModel()
			};
			var mgblock_image_url = elementor.imagesManager.getImageUrl( mgBlockImage );

			
		if (mgq_style === 'style1') {
			var mgq_style_one = true;
		} else if (mgq_style === 'style3') {
			var mgq_style_three = true;
		} else {
			var mgq_style_two = true;
		}
		#>
	
		<# if (mgq_style_two) { #>
			<div class="mg-qoute-single mgq-border-{{ settings.mgqborder_show }}-{{ settings.mgq_border_position }} mg-{{ settings.mgq_shadow_style }} mg-quote-{{ settings.mgq_style }} mg-icon-{{ settings.mgq_icon_position }} ">



			<div class="mg-quote-imf">
					
					<# if (settings.mgq_icon_type === 'image') { #>
					<figure class="mg-quote-img">						
					<img src="{{{ mgblock_image_url }}}" />
					</figure>
					<# } else { #>
					<div class="mgq-quote-icon">						
						<i class="{{{ settings.mgq_type_selected_icon.value }}}"></i>
					</div>
					<# } #>
				...
				<div class="mg-quote-details">
					<blockquote class="mg-blockquote">
						<p>
							<# if (settings.mgq_show_icon_top) { #>
								<span class="mcq-icon-top">
									<i class="{{{ settings.mgq_icon_top.value }}}"></i>
								</span>
							<# } #>
							{{{ settings.mgq_des }}}
							<# if (settings.mgq_show_icon_bottom) { #>
								<span class="mcq-icon-bottom">
									<i class="{{{ settings.mgq_icon_bottom.value }}}"></i>									
								</span>
							<# } #>
						</p>
					</blockquote>
				...
				<figcaption class="mg-quote-author">
					{{{ settings.mgq_name }}} <cite>{{{ settings.mgq_place }}}</cite>
				</figcaption>
			</div>
		<# } else if (mgq_style_one) { #>
			<div class="mg-qoute-single mgq-border-{{ settings.mgqborder_show }}-{{ settings.mgq_border_position }} mg-{{ settings.mgq_shadow_style }} mg-quote-{{ settings.mgq_style }}">
				<div class="mg-quote-details">
					<blockquote class="mg-blockquote">
						<p>
							<# if (settings.mgq_show_icon_top) { #>
