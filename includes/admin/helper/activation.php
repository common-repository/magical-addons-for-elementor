...
class Magcial_Addon_Activation_Class
{
	...
	public static function init()
	{
		...
		//	$library_data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get ($remote . '/wp-json/mg/v1/' . $endpoint . '/')), true);

		/*	
		$page_data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get ($remote_page . '/wp-json/wp/v2/mgaddon_pages')), true);
		$library_data['pages'] = $page_data;
		
		$page_data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get ('https://thepack.webangon.com/readypages//wp-json/wp/v2/mgaddon_pages')), true);
		$library_data['pages'] = $page_data;
		*/
		// version update
		if (!get_option('mgaddon_ready_items') || (get_option('mgaddon_version') != MAGICAL_ADDON_VERSION)) {
			$library_data = json_decode(wp_remote_retrieve_body(wp_safe_remote_get($remote . '/wp-json/mg/v1/' . $endpoint . '/', ['timeout' => 120])), true);
			if ($library_data) {
